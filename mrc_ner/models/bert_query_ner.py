...
from transformers import BertModel, BertPreTrainedModel, MegatronBertPreTrainedModel, MegatronBertModel, DebertaModel, DebertaPreTrainedModel, RobertaModel, ROBERTA_PRETRAINED_MODEL_ARCHIVE_LIST, RobertaConfig
...
class DebertaQueryNER(DebertaPreTrainedModel):
    ...
    def __init__(self, config):
        super(DebertaQueryNER, self).__init__(config)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None):
        """
        Args:
            input_ids: deberta input tokens, tensor of shape [seq_len]
            token_type_ids: 0 for query, 1 for context, tensor of shape [seq_len]
            attention_mask: attention mask, tensor of shape [seq_len]
        Returns:
            start_logits: start/non-start probs of shape [seq_len]
        ...
        deberta_outputs = self.deberta(input_ids, token_type_ids=token_type_ids, attention_mask=attention_mask)

        sequence_heatmap = deberta_outputs[0]  # [batch, seq_len, hidden]
        batch_size, seq_len, hid_size = sequence_heatmap.size()
...
class RobertaQueryNER(BertPreTrainedModel):
    def __init__(self, config):
        super(RobertaQueryNER, self).__init__(config)
