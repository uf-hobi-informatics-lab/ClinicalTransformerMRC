...
from models.bert_query_ner import BertQueryNER, MegatronQueryNER, DebertaQueryNER, RobertaQueryNER
from models.model_config import BertQueryNerConfig, DebertaQueryNerConfig
...
class BertLabeling(pl.LightningModule):
    def __init__(
        ...
        deberta_config = DebertaQueryNerConfig.from_pretrained(args.bert_config_dir,
                                                               hidden_dropout_prob=args.bert_dropout,
                                                               attention_probs_dropout_prob=args.bert_dropout,
                                                               mrc_dropout=args.mrc_dropout,
                                                               classifier_act_func = args.classifier_act_func,
        ...
        if self.args.model_type == "bert":
            ...
            self.model = MegatronQueryNER.from_pretrained(args.bert_config_dir, config=bert_config)
        elif self.args.model_type == "deberta":
            self.model = DebertaQueryNER.from_pretrained(args.bert_config_dir, config=deberta_config)
        elif self.args.model_type == "roberta":
            self.model = RobertaQueryNER.from_pretrained(args.bert_config_dir, config=bert_config)
        else:
            raise RuntimeError(f"model type should be bert, megatron, roberta or deberta but {self.args.model_type}")
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--model_type", type=str, default="bert", help="bert, megatron, roberta or deberta")
