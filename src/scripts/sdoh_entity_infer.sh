...
#
echo "Script started at: $(date +%Y-%m-%d\ %H:%M:%S.%N)"
...
DATASET_SIGN=/data/datasets/cheng/mrc-for-ner-medical/exp/model/gatortron-og-345m_deid_vocab_8_4_1e-5_20/label2idx.json
...
CUDA_VISIBLE_DEVICES=1 python3 ${REPO_PATH}/inference/mrc_ner_inference.py \
--data_dir ${DATA_DIR} \
--bert_dir ${BERT_DIR} \
--max_length ${MAX_LEN} \
...
echo "Script ended at: $(date +%Y-%m-%d\ %H:%M:%S.%N)"
